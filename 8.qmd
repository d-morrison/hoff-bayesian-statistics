...
format:
  html: default
  revealjs:
    output-file: 8-slides.html
  pdf:
    output-file: 8-handout.pdf
  docx:
    output-file: 8-handout.docx
---
...
have many schools, which we assume are samples from a population of schools. So
...
In the Chapter 6 notes ("a shortcut for thinking about full conditionals"), it should be mentioned that
...
Since the work is quite tedious, the derivations for the full
...
sampler; this is not done here, but dividing the samples into groups of e.g. 500 
...
For brevity, the analysis is not reproduced here as there aren't too many
...
The expectation is that $\text{Var}(y_{i, j} \mid \mu, \tau^2)$ will be bigger since it includes
both within- and between-group sampling variability.

### b

$\text{Cov}(y_{i_1, j}, y_{i_2, j} \mid \theta_j, \sigma^2)$ is expected to be zero
because, according to exchangeability, $y_{i, j}$ are conditionally i.i.d.
when $\theta_j, \sigma^2$ is known.

On the other hand, given the model, knowing about another $y_{i_1,
j}$ *does* appear to provide more information about $y_{i_2, j}$, and they are expected to
covary positively with each other. Specifically, $y_{i_1, j}$ seems to give more
information about what the mean $\theta_j$ is, and values from the
same $\theta_j$ are expected to be closer together (due to decreased variability). A more
formal mathematical justification cannot be provided here.
...
smt.df = smt.df %>%
  ...
  pivot_longer(cols = c(sigma2, mu, tau2), names_to = 'variable', values_to = 'value')
...
The prior estimates for $\mu$ and $\tau^2$ were fairly accurate, but the 
estimate for $\sigma^2$ was very far off. After this analysis, estimates are obtained
...
$R$ measures how much of the total variance in the data is between-group. The
prior didn't contain much information about this quantity, but after inference,
the expectation is that around 25\% of the variance comes from between group variance
($\tau^2$).
