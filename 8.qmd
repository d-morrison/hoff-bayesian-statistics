...
where we assume values from both groups have a common variance
$\epsilon_{i, j} \sim \text{i.i.d.}\; \mathcal{N}(0, \sigma^2)$.
...
unsurprising.
We treat the parameters as independent,
so $p(\mu, \delta, \sigma^2) = p(\mu) p(\delta) p(\sigma^2)$ where
...
as well as the probability that an individual from school 1 is higher than school 2,
which due to variance is a bit lower:
