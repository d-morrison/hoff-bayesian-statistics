...
format:
  html: default
  revealjs:
    output-file: 6-slides.html
  pdf:
    output-file: 6-handout.pdf
  docx:
    output-file: 6-handout.docx
---
...
\sim \mathcal{N}(\mu_n, \tau_n^2)$, where $\mu_n$ and $\tau_n^2$ are not
reproduced here (see 5.2).
...
at frequency zero [@marmer_gibbs]
...
sample sizes. However, the purpose of calculating the
autocorrelation of the minimum and maximum $\theta$ is unclear.
