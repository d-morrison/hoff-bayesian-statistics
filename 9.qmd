...
format:
  html: default
  revealjs:
    output-file: 9-slides.html
  pdf:
    output-file: 9-handout.pdf
  docx:
    output-file: 9-handout.docx
---
...
```{r}
swim = read.table('Exercises/swim.dat')
```

To specify the prior, the prior expectation of the $y$-intercept is set to 
23, and the prior expectation of the effect of training week by week is set to
0, so $\boldsymbol{\beta} = (23, 0)^T$.

No covariance with the $\beta$ coefficients is expected, but there is
uncertainty about the initial $\beta$ estimates. Specifically, to let 95% of the
uncertainty of the $y$-intercept fall in $[22, 24]$, $\Sigma_{0(1, 1)} = 1/4$ is set (so that $\pm$ 2 standard deviations is $\pm$ 1).
Training is expected to have a relatively mild effect on time, so
$\Sigma_{0(2, 2)} = 0.1$ is set, which is an arbitrarily chosen small variance.

For the expectation of the variability of measurements, similarly set 
...
With the posterior predictive dataset, swimmer 1 is the fastest 
about 65% of the time by week 13, so the recommendation is that swimmer 1 race.

## 9.3

```{r}
crime = read.table('Exercises/crime.dat', header = TRUE)
```
...
```{r}
y = crime$y
X = crime %>% dplyr::select(-y) %>% as.matrix
