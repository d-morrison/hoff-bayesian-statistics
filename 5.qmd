...
format:
  html: default
  revealjs:
    output-file: 5-slides.html
  pdf:
    output-file: 5-handout.pdf
  docx:
    output-file: 5-handout.docx
---
...
that the normal family is conjugate to the normal sampling model. 
:::

Let $\theta \mid \sigma^2 \sim \mathcal{N}(\mu_0, \tau_0^2)$ (interpretations of the prior parameters comes later):
...
df = as.data.frame(d) |>
  ...
  pivot_longer(cols = -theta, names_to = 'sigma2', values_to = 'density') |>
  mutate(theta = as.numeric(theta), sigma2 = as.numeric(sigma2))
...
With "significant algebra" [@wood2009], you can show that inference this way results in
...
The exact algebra is omitted here, but note that once the
exponential terms are combined (and the matrix multiplication in the second exp), there
...
# Means and confidence intervals of the \sigma^2 can easily be computed, but for
# brevity, only \theta is output
...
The derivation of $p(\sigma^2 \mid y_1, \dots, y_n)$ is as follows:
...
This is apparently non-trivial, and requires expanding the quadratic terms in the $\text{exp}$ terms. This step is skipped for now.
...
After some poking around, this is a normal-inverse-chi-squared distribution [@reif2010].
...
normal likelihood reparameterization trick from @zivot_mle (page 3):
