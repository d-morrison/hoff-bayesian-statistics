...
format:
  html: default
  revealjs:
    output-file: 3-slides.html
  pdf:
    output-file: 3-handout.pdf
  docx:
    output-file: 3-handout.docx
---
...
> Note: The general derivation of posterior probabilities for uniform
> prior is skipped here, since a uniform prior is also a $\text{Beta}(a, b)$.
...
the next observation $\tilde{Y}$. 

::: notes
Intuitively we should predict $\tilde{Y} = 1$
...
::: notes
See @stackexchange_confidence for a really interesting discussion about the difference between Bayesian and frequentist confidence intervals.
:::
...
entire curve. The analytical calculation for these is not straightforward; computationally, using a discretized density, a "trial and error" approach or a neat procedure detailed in Exercise 4.7 (c) can be used.
...
df.long = df |>
  pivot_longer(cols = -theta, names_to = 'dist', values_to = 'value')
...
For consistency, these will be rewritten as done in Chapter 1, where $\theta_0 = a
...
So the posterior is another weighted mixture. However, the 
weights of the relative components are not preserved (neither are they in the Beta mixtures
above).

### b

Not specified whether the exponential family parameterization or
the standard parameterization should be used. The standard parameterization will be used.
...
Note it was a technicality that $t(y) = -(y^2)$ was chosen. The exponential family
could have been parameterized such that $t(y) = y^2$, and the negative is
...
the Jeffreys' prior exercises next up), see @zhu2011.
...
Gamma density, it follows that $f(\theta, y)$ can be normalized such that it 
represents a valid posterior density for $\theta$. Actually calculating
$f(\theta, y) / \int f(\theta, y) \; d\theta$ yields such a Gamma density.
