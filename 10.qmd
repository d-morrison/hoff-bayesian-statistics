...
Delta as a tradeoff between rate of approach towards the HPD region versus overshooting
(like learning rate in gradient descent, etc)
...
When $\theta_0 > \delta$,
$\tilde{\theta} \in [\theta_0 - \delta, \theta_0 + \delta] > 0$,
so $\theta_1 \sim \text{uniform}(\theta_0 - \delta, \theta_0 + \delta)$ and
...
For convenience let $\theta_i = P(Y_i = 1 \mid \alpha, \beta, x_i)$.
Now we solve for $\theta_i$ in our model
