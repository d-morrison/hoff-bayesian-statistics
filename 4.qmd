...
format:
  html: default
  revealjs:
    output-file: 4-slides.html
  pdf:
    output-file: 4-handout.pdf
  docx:
    output-file: 4-handout.docx
---
...
algorithmically sampling from a Normal distribution given just pseudo-random number generators, see the Box-Muller transform [@goodman2005].
However,
...
combined = as.data.frame(cbind(prior, posterior)) |>
  tibble::rownames_to_column('n') |>
  pivot_longer(cols = -n, names_to = 'type', values_to = 'value')
...
Because exercises 4.3 and 4.8 address this, this section is skipped for now.
...
```{r}
menchild30bach = scan('Exercises/menchild30bach.dat')
...
empirical distribution peaks strongly around 0. This suggests an error in the model fit.
